...
public final class Connection: NSObject, StreamDelegate {
	...
	struct WriteItem {
		...
		let type: FrontendMessageType
		/// The body of the message
		let buffer: WriteBuffer
		/// The callback to call when the item has been written
		let completion: (() -> Void)?
		
		init(type: FrontendMessageType, buffer: WriteBuffer, completion: (() -> Void)?) {
			self.type = type
			self.buffer = buffer
	...
	/// A queue of writes to perform as the socket is ready to write them
	...
	/// Trigger a write if the socket is ready
	private func write() {
	...
	/// Trigger a read if the socket has data available
	private func read() {
		if #available(OSX 10.12, *) {
			dispatchPrecondition(condition: .onQueue(self.queue))
		}
